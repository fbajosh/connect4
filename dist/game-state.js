...
function countDirection(board, startCol, startRow, colStep, rowStep, player) {
    let count = 0;
    let col = startCol;
    let row = startRow;
...
function hasFour(board, col, row, player) {
    const directions = [
        [
            1,
            0
    ...
    return directions.some(([colStep, rowStep])=>{
        const total = countDirection(board, col, row, colStep, rowStep, player) + countDirection(board, col, row, -colStep, -rowStep, player) - 1;
        return total >= 4;
    });
}
function computeWinningColumns(board, heights, player) {
...
export function parseSequence(sequence) {
    ...
    for(let moveIndex = 0; moveIndex < sequence.length; moveIndex += 1){
        ...
        if (!Number.isInteger(col) || col < 0 || col >= WIDTH) {
            return {
                error: `Move ${moveIndex + 1} is not a valid column. Use digits 1 through 7.`,
                ok: false
            };
        }
        if (winner !== 0) {
            return {
                error: `Move ${moveIndex + 1} happens after the game is already over.`,
                ok: false
            };
        }
        const row = heights[col];
        if (row >= HEIGHT) {
            return {
                error: `Move ${moveIndex + 1} tries to play into a full column.`,
                ok: false
            };
        }
