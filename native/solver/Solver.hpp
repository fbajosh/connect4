...
class Solver {
 private:
  ...
  std::atomic<int> currentMoveColumn;
  std::atomic<int> currentMoveIndex;
  std::atomic<int> legalMoveCount;
  int columnOrder[Position::WIDTH]; // column exploration order

  void setCurrentMove(int column, int moveIndex, int moveCount) {
    currentMoveColumn.store(column, std::memory_order_relaxed);
    currentMoveIndex.store(moveIndex, std::memory_order_relaxed);
    legalMoveCount.store(moveCount, std::memory_order_relaxed);
  }

  void resetCurrentMove() {
    currentMoveColumn.store(0, std::memory_order_relaxed);
    currentMoveIndex.store(0, std::memory_order_relaxed);
    legalMoveCount.store(0, std::memory_order_relaxed);
  }
 ...
 public:
  ...
  int getCurrentMoveColumn() const {
    return currentMoveColumn.load(std::memory_order_relaxed);
  }

  int getCurrentMoveIndex() const {
    return currentMoveIndex.load(std::memory_order_relaxed);
  }

  int getLegalMoveCount() const {
    return legalMoveCount.load(std::memory_order_relaxed);
  }
