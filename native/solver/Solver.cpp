...
int Solver::negamax(const Position &P, int alpha, int beta) {
  ...
  nodeCount.fetch_add(1, std::memory_order_relaxed); // increment counter of explored nodes
...
std::vector<int> Solver::analyze(const Position &P, bool weak) {
  ...
  for (int col = 0; col < Position::WIDTH; col++)
    if (P.canPlay(col)) {
      moveIndex++;
      setCurrentMove(col + 1, moveIndex, moveCount);
...
// Constructor
Solver::Solver() : nodeCount{0}, currentMoveColumn{0}, currentMoveIndex{0}, legalMoveCount{0} {
